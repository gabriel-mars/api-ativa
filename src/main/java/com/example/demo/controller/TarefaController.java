...
import com.example.demo.model.service.TarefaService;
...
public class TarefaController {
    ...
    @PostMapping("/tarefa")
    public ResponseEntity<?> saveTarefa(@RequestParam("token") String token, @RequestBody TarefaEntity tarefa){
    ...
    public ResponseEntity<?> findTarefaById(@RequestParam("token") String token, @PathVariable Long id) {
        if(!token.isBlank() || !token.isEmpty()){
            try {
                UsuarioEntity user = usuarioService.findByToken(token);
                if(user != null){
                    TarefaEntity tarefa = service.findById(id);
