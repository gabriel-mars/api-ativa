...
import com.example.demo.model.service.ConvidadoService;
...
public class ConvidadoController {
    ...
    public ResponseEntity<?> findConvidadoById(@RequestParam("token") String token, @PathVariable Long id) {
        if(!token.isBlank() || !token.isEmpty()){
            try {
                UsuarioEntity user = usuarioService.findByToken(token);
                if(user != null){
                    ConvidadoEntity convidado = service.findById(id);
                    return ResponseEntity.status(HttpStatus.ACCEPTED).body(convidado);
                } else {
                    return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Necessário autenticação.");
                }
